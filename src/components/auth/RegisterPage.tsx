...
export function RegisterPage({ onToggle, onBack }: RegisterPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!matric.trim()) { toast.error('Matric number is required');  return; }
    ...
    setLoading(true);
    try {
      await signUp(email.trim(), password, name.trim(), role, matric.trim(), phone.trim());
    ...
    <div className="auth-page">
      <div
        style={{
          ...
          background: 'radial-gradient(ellipse 60% 50% at 70% 50%, rgba(37,99,235,0.08) 0%, transparent 70%)',
        }}
      />

      <div className="auth-card" style={{ position: 'relative', maxWidth: 500 }}>
        <button
        ...
        <form onSubmit={handleSubmit}>
          ...
          {/* Matric + Phone side by side */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.875rem' }}>
            <div className="form-group">
              <label className="label">Matric Number <span style={{ color: 'var(--danger)' }}>*</span></label>
              <input
                type="text"
                className="input-field"
                placeholder="e.g. AF210001"
                value={matric}
            ...
            <div style={{ display: 'flex', gap: '0.75rem', flexDirection: 'column' }}>
              {ROLES.map(r => (
                ...
                >
                  ...
                  />
                  <div>
                    <div style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: '0.9rem', color: role === r.value ? 'var(--blue-300)' : 'var(--text-primary)' }}>
                      {r.label}
                    </div>
        ...
        <p style={{ textAlign: 'center', fontSize: '0.875rem', color: 'var(--text-secondary)' }}>
          ...
          <button
            onClick={onToggle}
            style={{ background: 'none', border: 'none', color: 'var(--blue-400)', cursor: 'pointer', fontWeight: 600, fontFamily: 'var(--font-display)' }}
          >
            Sign in
          </button>
        </p>
